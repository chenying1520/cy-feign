...
public class FallbackFactoryTest {
  ...
  public void fallbackFactory_example_lambda() {
    ...
    TestInterface api = target(cause -> () -> {
      Assertions.assertThat(cause).isInstanceOf(FeignException.class);
      return ((FeignException) cause).status() == 500 ? "foo" : "bar";
    });

    Assertions.assertThat(api.invoke()).isEqualTo("foo");
    Assertions.assertThat(api.invoke()).isEqualTo("bar");
  }
  ...
  public void fallbackFactory_example_retro() {
    ...
    Assertions.assertThat(api.invoke()).isEqualTo("status 500 reading TestInterface#invoke()");
  }
  ...
  public void defaultFallbackFactory_delegates() {
    ...
    Assertions.assertThat(api.invoke())
        .isEqualTo("foo");
  }
  ...
  public void defaultFallbackFactory_logsAtFineLevel() {
    ...
    Logger logger = new Logger("", null) {
      ...
      public void log(Level level, String msg, Throwable thrown) {
        ...
        Assertions.assertThat(msg).isEqualTo("fallback due to: status 500 reading TestInterface#invoke()");
        Assertions.assertThat(thrown).isInstanceOf(FeignException.class);
      }
    };
    logger.setLevel(Level.FINE);

    target(new FallbackFactory.Default<>(() -> "foo", logger)).invoke();
    Assertions.assertThat(logged.get()).isTrue();
  }
